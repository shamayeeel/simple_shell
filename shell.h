...
/**
 *struct passinfo - holds pseudo-parameters to pass into a function,
 ...
 *@env: linked list representing the local copy of the environment
 *@environ: custom modified copy of the environment from LL env
 *@history: the history node
 *@alias: the alias node
 *@env_changed: set to 1 if environ was changed
 *@status: the return status of the last executed command
 *@cmd_buf: address of pointer to cmd_buf, set to 1 if chaining
 *@cmd_buf_type: CMD_type represents ||, &&, or ;
...
typedef struct passinfo
{
    ...
    int cmd_buf_type; /* CMD_type represents ||, &&, ; */
    int readfd;
    int histcount;
} info_t;
